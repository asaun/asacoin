...
const int ASACOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString ASACOIN_IPC_PREFIX("asacoin:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(ASACOIN_IPC_PREFIX, Qt::CaseInsensitive)) // asacoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseAsacoinURI(arg, &r) && !r.address.isEmpty())
            {
                CAsacoinAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(ASACOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(ASACOIN_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(ASACOIN_IPC_PREFIX, Qt::CaseInsensitive)) // asacoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseAsacoinURI(s, &recipient))
            {
                CAsacoinAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
                ...
                emit message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Asacoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CAsacoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Insecure payments to custom asacoin addresses are not supported
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            emit message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(AsacoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
