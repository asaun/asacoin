...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars ASACOIND and ASACOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

ASACOIND=${ASACOIND:-${1}/asacoind}
CLI=${ASACOINCLI:-${1}/asacoin-cli}
...
$ASACOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$ASACOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
