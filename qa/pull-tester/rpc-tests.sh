...
export ASACOINCLI=${BUILDDIR}/qa/pull-tester/run-asacoin-cli
export ASACOIND=${REAL_ASACOIND}
...
if [ "x${ENABLE_ASACOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and asacoind must all be enabled"
fi
