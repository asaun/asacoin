...
*Setup instructions for a gitian build of Asacoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Asacoin
...
- [Building Asacoin](#building-asacoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-asacoin-host/README.md).
...
Clone the git repositories for asacoin and gitian and then checkout the asacoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/asacoin/asacoin
cd asacoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the asacoin repository
...
To build Asacoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the asacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/asacoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/asacoin/asacoin
...
For example:
```bash
URL=https://github.com/laanwj/asacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit asacoin=${COMMIT} --url asacoin=${URL} ../asacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit asacoin=${COMMIT} --url asacoin=${URL} ../asacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit asacoin=${COMMIT} --url asacoin=${URL} ../asacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/asacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/asacoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/asacoin-build.assert
```
...
[asacoin/gitian.sigs](https://github.com/asacoin/gitian.sigs/) repository, or if that's not possible create a pull
